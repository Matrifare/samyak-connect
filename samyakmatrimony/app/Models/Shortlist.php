...
class Shortlist extends Model
{
    ...
    protected array $fillable = ['from_id', 'to_id', 'created_at'];
    ...
    public function add(string $userId, string $profileId): array
    {
        ...
        $query = "INSERT INTO {$this->table} (from_id, to_id, created_at) VALUES (?, ?, NOW())";
        $id = $this->db->insert($query, [$userId, $profileId]);

        if ($id) {
            return ['success' => true, 'message' => 'Profile shortlisted'];
        }

        return ['success' => false, 'message' => 'Failed to shortlist'];
    }

    public function remove(string $userId, string $profileId): array
    {
        $query = "DELETE FROM {$this->table} WHERE from_id = ? AND to_id = ?";
    ...
    {
        $query = "SELECT COUNT(*) as count FROM {$this->table} WHERE from_id = ? AND to_id = ?";
    ...
    public function getList(string $userId, int $page = 1, int $limit = 20): array
    {
        ...
        $query = "SELECT s.*, r.username, r.photo1, r.photo1_approve, r.gender, r.matri_id,
                  ...
                  FROM {$this->table} s
                  LEFT JOIN register_view r ON s.to_id = r.matri_id
                  WHERE s.from_id = ?
    ...
    public function getWhoShortlistedMe(string $userId, int $page = 1, int $limit = 20): array
    {
        ...
        $query = "SELECT s.*, r.username, r.photo1, r.photo1_approve, r.gender, r.matri_id,
                         ...
                         TIMESTAMPDIFF(YEAR, r.birthdate, CURDATE()) as age
                  FROM {$this->table} s
                  LEFT JOIN register_view r ON s.from_id = r.matri_id
                  WHERE s.to_id = ?
                  ORDER BY s.created_at DESC
                  LIMIT ?, ?";
    ...
    {
        $query = "SELECT COUNT(*) as count FROM {$this->table} WHERE from_id = ?";
        $result = $this->db->selectOne($query, [$userId]);
        return $result['count'] ?? 0;
    }

    public function getShortlistedMeCount(string $userId): int
    {
        $query = "SELECT COUNT(*) as count FROM {$this->table} WHERE to_id = ?";
        $result = $this->db->selectOne($query, [$userId]);
        return $result['count'] ?? 0;
    }
}
