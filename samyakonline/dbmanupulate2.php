...
if (isset($_POST['actionfunction']) && $_POST['actionfunction'] != '') {
    ...
    $page = $_POST['page'];
    $limit = (isset($_SESSION['result_limit']) && !empty($_SESSION['result_limit'])) ? $_SESSION['result_limit'] : 8;
    $adjacent = 2;
    ...
    if($orderby != '' && $orderby == 'premium') {
        ...
        $sql = "select DISTINCT firstname,username,m_status, last_login,photo1,photo1_approve,photo_protect,photo_view_status,matri_id,samyak_id,photo_pswd,gender,email,status,profile_text,birthdate,height,religion_name,caste_name,mtongue_name,family_city,edu_name,city_name,country_name,state_name,ocp_name,logged_in,index_id,reg_date,photo_view_status,p.p_plan,p.pactive_dt from register_view r INNER JOIN payments p ON matri_id=pmatri_id where status NOT IN ('Inactive', 'Suspended') and matri_id NOT IN (select block_by from block_profile where block_to='" . $_SESSION['user_id'] . "') $a $b $c $d $e $f_level $f $g $h $i $j $k $family $l $m $n $q $s $sqlOtherCaste $sqlSpecialCase $condition $samyak $samyakSearch $searchByProfileId $t $u $v $x $y $z $a1 $r $o limit $start,$limit";
        ...
        $sql = "select DISTINCT firstname,username,m_status, last_login,photo1,photo1_approve,photo2,photo2_approve,photo3,photo3_approve,photo4,photo4_approve,photo_protect,photo_view_status,matri_id,samyak_id,photo_pswd,gender,email,status,hor_photo,hor_check,r.video,r.video_url,profile_text,video_approval,birthdate,height,religion_name,caste_name,mtongue_name,family_city,edu_name,city_name,country_name,state_name,ocp_name,logged_in,index_id,reg_date,photo_view_status,p.p_plan,p.pactive_dt from register_view r INNER JOIN payments p ON r.matri_id = p.pmatri_id where status NOT IN ('Inactive', 'Suspended') and matri_id NOT IN (select block_by from block_profile where block_to='" . $_SESSION['user_id'] . "') $a $b $c $d $e $f_level $f $g $h $i $j $k $family $l $m $n $q $s $sqlOtherCaste $sqlSpecialCase $condition $samyak $samyakSearch $searchByProfileId $t $u $v $x $y $z $a1 $r $o limit $start,$limit";
        ?>
        <div class="result-status" >
            <p>We found <span class="text-primary font700"><?= $rows ?></span> results.</p>
        </div>
        <?php
    }
