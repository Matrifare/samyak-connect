...
if (!empty($_POST['interest_type']) && $_POST['interest_type'] == 'sent') {
 ...
 WHERE register_view.matri_id=expressinterest.ei_sender and expressinterest.ei_receiver='$mid' AND expressinterest.trash_sender='No'
  and expressinterest.trash_receiver='No' and expressinterest.receiver_response='Pending' and register_view.status <> 'Suspended' GROUP BY expressinterest.ei_sender"));
}
if ($rows > 0) {
    if (!empty($_POST['interest_type']) && $_POST['interest_type'] == 'sent') {
        $sql = "SELECT DISTINCT r.firstname, r.username, r.m_status, r.last_login, r.photo1, r.photo1_approve, r.photo_protect,
                ...
                FROM expressinterest e LEFT JOIN register_view r ON e.ei_receiver = r.matri_id 
                    WHERE e.ei_sender = '$mid' AND e.trash_sender = 'No' AND e.trash_receiver = 'No' 
                      AND e.receiver_response = 'Pending' AND r.status <> 'Suspended' ORDER BY e.ei_id DESC LIMIT $start,$limit";
    } else {
        $sql = "SELECT DISTINCT r.firstname, r.username, r.m_status, r.last_login, r.photo1, r.photo1_approve, r.photo_protect,
                ...
                r.family_city, r.ocp_name, r.logged_in, r.index_id, e.ei_sent_date, e.ei_message, e.ei_id
                            ...
                            <div class="col-xs-8 col-sm-6">
                                <div class="action-btn">
                                    ...
                                    if (!empty($_POST['interest_type']) && $_POST['interest_type'] == 'received') {
                                        ?>
                                        <button id="accept<?= $Row->ei_id ?>" class="btn btn-sm btn-primary btn-shrink"
                                        ...
                                        <button id="hold<?= $Row->ei_id ?>" class="btn btn-sm btn-success btn-shrink"
                                                onclick="onHoldInt('<?= $Row->ei_id ?>')">Decide Later
                                        </button>
                                        <?php
                                    } else { ?>
                                        <!--<button id="privilege<?/*= $Row->ei_id */?>" class="btn btn-sm btn-success btn-shrink"
                                                onclick="convertToPrivilege(,,);">
                                            <i class="fa fa-star-o"></i>
                                            Convert to Privilege
                                        </button>-->
                                        <button id="remind<?= $Row->ei_id ?>" class="btn btn-sm btn-primary btn-shrink"
                                                onclick="sendReminder('<?= $Row->ei_id ?>', 'reminder')">Send Reminder
                                        </button>
                                        <button id="reject<?= $Row->ei_id ?>" class="btn btn-sm btn-danger btn-shrink"
            ...
            <div class="row">
                ...
                <div class="col-sm-12">
                    <ul class="paging mb-5">
                        <?php
                        pagination($limit, $adjacent, $rows, $page, $_POST['interest_type'], 'pending');
                        ?>
                    </ul>
                </div>
    ...
    <script type="text/javascript">
        $("#countWithExpressInterest").html("Pending - <?= $rows ?>");
    </script>
    <?php
} else { ?>
    <div class="tab-content-inner">
    ...
    <script type="text/javascript">
        $("#countWithExpressInterest").html("Pending - 0");
    </script>
<?php }
?>