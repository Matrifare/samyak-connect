...
$mid = !empty($_SESSION['user_id']) ? $_SESSION['user_id'] : 0;
$from_id = !empty($_POST['frmid']) ? mysqli_real_escape_string($DatabaseCo->dbLink, $_POST['frmid']) : 0;
$name = !empty($_POST['name']) ? mysqli_real_escape_string($DatabaseCo->dbLink, $_POST['name']) : 0;
...
if (!empty($mid)) {
        ...
        if ($num_block > 0) {
            ...
            <form name="MatriForm" id="MatriForm" class="form-horizontal" action="premium_member" method="post">
                <div class="form-group">
                    <div class="col-sm-12">
                        <?php
                        echo "<p style='margin:10px 0px; color:red;' class='text-center'><b>This member has blocked you.<br/>You can't express your interest.</b></p>
        ...
        <?php } else {
            ...
            if ($num_block < 1) {
                ...
                if ($isInterestCheckPassed == FALSE) {
                        ...
                        <form name="MatriForm" id="MatriForm" class="form-horizontal" action="premium_member"
                            ...
                            <div class="modal-body xxl-16 xl-16 l-16 m-16 s-16 xs-16 ne_pad_btm_10px ne_pad_tp_10px">
                                <div class="xxl-16 xl-16 l-16 m-16 s-16 xs-16 padding-lr-zero">
                                    <p class="text-left">
                                        The settings of privacy of this member does not allow you to send the express
                                        interest to him/her.
                                        <br/>
                    ...
                    <div class="row">
                        <div class="col-xs-12 font-16" style="font-weight: bold;" id="ExpressLabel">
                    ...
                    <form name="MatriForm" id="MatriForm" class="form-horizontal" action="" method="post">
                        <div class="row">
                            <div class="col-xs-12 mt-10 text-center">
                                <input type="hidden" name="ExmatriId" id="ExmatriId"
                                       value="<?php echo $from_id; ?>"/>
                                <input name="exp_interest" class="radio-inline" type="radio" checked
                                       value="You seem to be the kind of person who suits our family. Please accept if you are interested."/>
                                Are you interested in <?= $name ?>'s Profile?
                            </div>
                        </div>
                ...
                if ($expressResult->receiver_response == 'Pending') {
                    if ($expressResult->ei_sender == $mid &&
                        ($expressResult->trash_receiver == 'No' || $expressResult->trash_sender == 'No')) { ?>
                    ...
                    if ($expressResult->ei_sender == $mid) { ?>
                        ...
                        <div class="action-btn mt-10">
                            <button class="btn btn-xs btn-danger btn-shrink" data-dismiss="modal"
                                    onclick="window.location.href='view_profile?userId=<?= $expressResult->ei_receiver ?>&view_contact=yes';">
                                View Contact
                            </button>
                            <button class="btn btn-xs btn-primary btn-shrink" data-dismiss="modal"
                                    onclick="window.location.href='view_profile?userId=<?= $expressResult->ei_receiver ?>&send_message=yes';">
                                Message Now
                            </button>
                        </div>
                    <?php } else { ?>
                        ...
                        <div class="action-btn mt-10">
                            <button class="btn btn-xs btn-danger btn-shrink" data-dismiss="modal"
                                    onclick="window.location.href='view_profile?userId=<?= $expressResult->ei_sender ?>&view_contact=yes';">
                                View Contact
                            </button>
                            <button class="btn btn-xs btn-primary btn-shrink" data-dismiss="modal"
                                    onclick="window.location.href='view_profile?userId=<?= $expressResult->ei_sender ?>&send_message=yes';">
                                Message Now
                            </button>
                        </div>
                    <?php }
                    ...
                    if ($expressResult->ei_sender == $mid) { ?>
                        <span style="color: orange; font-weight: bold"> You had already sent the express interest and it has been neither accepted nor rejected by this member, instead, the member chosen to decide later.</span>
                    <?php } else { ?>
                        <span style="color: orange; font-weight: bold"> You have chosen to decide later this express interest.</span>
                        <button class="btn btn-xs btn-primary btn-shrink"
        ...
        <form name="MatriForm" id="MatriForm" class="form-horizontal" action="premium_member" method="post">
            <div class="modal-body xxl-16 xl-16 l-16 m-16 s-16 xs-16 ne_pad_btm_10px ne_pad_tp_10px">
                <div class="xxl-16 xl-16 l-16 m-16 s-16 xs-16 padding-lr-zero">
                    ...
                    <span>
                    	<h4 class="text-center">&nbsp;&nbsp;You are not a paid member, Please upgrade your membership to express the interest.</h4>
                    </span>
                    </h3>
    ...
    <form name="MatriForm" id="MatriForm" class="form-horizontal" action="login" method="post">
        <div class="form-group">
            <div class="col-sm-12 text-center">
        ...
        <div class="form-group">
            <div class="col-sm-12 text-center">
                <button class="btn btn-danger btn-sm" onclick="window.location.href='login'">Login Now</button>
            </div>
        </div>
    </form>
    ...
    <script type="text/javascript">
        $('#go_go_go').click(function () {
            ...
            $.ajax({
                ...
                cache: false,
                beforeSend: function () {
                    $("#go_go_go").css('letter-spacing', '1px').html("<i class='fa fa-spinner fa-spin'></i> Please wait...");
                },
